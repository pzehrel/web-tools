...
**仓库为 pnpm workspace monorepo**：前端在 `apps/web/`（`@web-tools/web`），仓库根只放 workspace 配置、ESLint、husky、wrangler 和仓库级脚本（`scripts/release.mjs`）。未来 `apps/server`（Hono 后端）与 `apps/admin` 见 `docs/BACKEND.md`，暂未创建。

## 技术栈

- **构建**：Vite 7（端口 1420，`strictPort`）+ `vite-react-ssg`（SSG 预渲染），配置在 `apps/web/vite.config.ts`
- **框架**：React 19 + TypeScript（strict，`tsc -b` 项目引用模式）+ react-router v8
- **样式**：Tailwind CSS v4（经 `@tailwindcss/postcss`）+ SCSS（`sass-embedded`）
- **UI 组件**：shadcn/ui（new-york 风格，`apps/web/components.json` 已配置，`@` 别名指向 `apps/web/src`）
- **图标**：lucide-react
- **Lint**：ESLint 9 + `@antfu/eslint-config`（v9，含 react 规则集），配置在根 `eslint.config.ts`
- **包管理**：pnpm workspace（见 `pnpm-workspace.yaml` 的 `packages: ['apps/*']`）
- **部署**：Cloudflare Workers 静态资产（根 `wrangler.jsonc`，assets 指向 `apps/web/dist`），GitHub Actions 自动部署

## 常用命令（仓库根执行）

```bash
pnpm dev        # 开发服务器 http://localhost:1420（CSR 模式，代理到 apps/web）
pnpm build      # apps/web 内 tsc -b && SSG 构建（产出 apps/web/dist/，含各路由预渲染 HTML）
pnpm preview    # 预览构建产物
pnpm lint       # ESLint 检查（根配置，覆盖全仓库）
pnpm lint:fix   # ESLint 自动修复
pnpm typecheck  # pnpm -r 逐包类型检查
pnpm test       # pnpm -r 逐包测试
...
2. **不新建 `.css` 文件**，样式全部写 `.scss`。Tailwind 入口是 `apps/web/src/index.scss`。
   - 注意：`@tailwindcss/vite` 插件只处理 `.css` 文件，所以本项目用
     `@tailwindcss/postcss`（在 `apps/web/vite.config.ts` 的 `css.postcss` 中配置），不要改回去。
...
3. **不写死颜色**（禁止 `bg-purple-500`、`#7c3aed` 这类）。所有颜色走 shadcn 语义令牌
   ...
   换肤只改 `apps/web/src/index.scss` 里的 `:root` / `.dark` 色板。
...
7. shadcn/ui 组件（`apps/web/src/components/ui/`）允许同时导出组件和常量
   （已在根 `eslint.config.ts` 中关闭该目录的 `react-refresh/only-export-components`）。
...
- `pnpm build` = `apps/web` 内 `tsc -b` + `node scripts/ssg-build.mjs`（vite-react-ssg 预渲染所有路由到 `apps/web/dist/`）。
  - 不用 vite-react-ssg 自带 CLI：其 yargs 命令在部分 Node 版本下异步 handler 是
    fire-and-forget，进程会静默退出不构建。`apps/web/scripts/ssg-build.mjs` 直接 `await build()`。
- `wrangler.jsonc`（仓库根）：assets-only Worker（无 Worker 代码），`apps/web/dist/` 直接作为静态资产；
  `html_handling: 'drop-trailing-slash'`（与 canonical 的无尾斜杠 URL 对齐）。
- 生产域名：`https://webtools.pzehrel.com`（Cloudflare Workers 自定义域名）
  （写在 `apps/web/src/components/seo.tsx` 的 `SITE_URL`、`apps/web/public/sitemap.xml`、
  `apps/web/public/robots.txt`，换域名时三处一起改）。
...
- **路由在 `apps/web/src/main.tsx`**：数据路由对象 + `ViteReactSSG`。每个路由有显式 `id`，
...
- **`apps/web/vendor/react-router-dom/`**：兼容垫片（file: 依赖）。vite-react-ssg 按
...
- **SEO**：每页用 `apps/web/src/components/seo.tsx` 的 `<Seo>` 组件写 title/description/OG/canonical；
  `apps/web/index.html` 模板里不放静态 title/description（避免与 helmet 输出重复）。
- 主题防闪烁：`apps/web/index.html` 有内联脚本在 React 挂载前挂 `.dark` class。

## 目录结构

```
apps/web/                 # 前端应用（@web-tools/web）
  src/
...
scripts/
  release.mjs           # CalVer 发版脚本（仓库级）
docs/DESIGN.md          # 设计规范（改 UI 前必读）
docs/BACKEND.md         # 可选后端架构决策（未实施）
wrangler.jsonc          # Workers 静态资产部署配置（仓库根）
...
1. 在 `apps/web/src/tools/index.ts` 的 `tools` 数组登记元信息（id、名称、描述、lucide 图标、`accentClass`）。
2. 在 `apps/web/src/tools/` 下实现工具页面组件，页面顶部加 `<Seo>`。
3. 在 `apps/web/src/main.tsx` 的路由表登记 `/tools/:id`：**路由加显式 `id` 并同步 `ROUTE_IDS`**，
   否则水合会出问题。
4. 把 `status` 从 `planned` 改为 `ready`。
5. `apps/web/public/sitemap.xml` 加新路由的 URL。
